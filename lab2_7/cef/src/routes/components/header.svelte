  ...
  onMount(() => {
    const setActivePage = () => {
      if ($activePage >= 0 && $activePage <= 1) {
        const headerTypes = document.getElementById('headerTypes');
        
        if (headerTypes) {
          const list = headerTypes.querySelectorAll('.element');
          if (list.length > 0) {
            list[$activePage].classList.add('active');
          }
        }
      }
    }
...
<section id="header">
  <div class="container">
    ...
    <div class="logo">
      <a href="/"><img src="{logotype}" alt="Golobe"></a>
    </div>
    <div class="buttons">
      <button>Login</button>
